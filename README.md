...
**[Open the app](https://eclinician.hadijahk.com/login)** ·
[API docs](https://eclinician.api.hadijahk.com/swagger-ui.html) ·
[health check](https://eclinician.api.hadijahk.com/api/health) ·
...
**Run:** [the app](https://eclinician.hadijahk.com/login), role by role — full runbook
...
- [API reference](docs/api.md) generated from the controllers
  ([Swagger UI](https://eclinician.api.hadijahk.com/swagger-ui.html)), so it cannot drift.
...
- **No secrets committed** — database credentials, `JWT_SECRET` and the AI key all injected
  as environment variables from a `.env` that lives only on the server
...
**[eclinician.hadijahk.com](https://eclinician.hadijahk.com/login)** ·
[API health](https://eclinician.api.hadijahk.com/api/health) ·
[docs/deployment.md](docs/deployment.md) · stack in [deploy/](deploy/) ·
pipeline in [deploy.yml](.github/workflows/deploy.yml)

| Container | What it is | Live |
|---|---|---|
| `eclinician-web` | React build, served by Caddy | [open the app](https://eclinician.hadijahk.com/login) |
| `eclinician-api` | Spring Boot API | [health check](https://eclinician.api.hadijahk.com/api/health) |
| `eclinician-db` | PostgreSQL 16 | reachable only from the API |

- A Contabo VPS running Docker Compose, behind a Caddy proxy that handles HTTPS
  (Let's Encrypt certificates, renewed automatically).
- **Every merge to `main` deploys itself:** GitHub Actions builds both images, pushes them to
  GHCR, and the server pulls and restarts. The server never builds.
- Always on — no free-tier sleep, no cold start before a demo.
- **Nothing secret is in the repository** — credentials sit in a `.env` on the server only.
...
| [Deployment](docs/deployment.md) | Local, Docker, Contabo, migrations |
